...
*NOTE:* Due to Invidious API changes, Elfeed Tube is currently unable to obtain video thumbnails or descriptions. See [[https://github.com/karthink/elfeed-tube/issues/35][issue #35]] for further details and discussion on alternative approaches. There is now an experimental =yt-dlp= based fetch method provided as an alternative. See [[#fetch-method][this section]] of this README for details.
...
** Fetch method
Due to changes in the availability of Invidious API services, Elfeed Tube provides a (currently experimental) feature to leverage =yt-dlp= from the user's system for download of duration, description, thumbnail and chapter video information. If =yt-dlp= is found on the emacs =exec-path= at Elfeed Tube startup, the =yt-dlp= fetch method will be used by default. This can be changed by customizing =elfeed-tube-use-ytdlp-p=.
